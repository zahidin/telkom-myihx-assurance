...
describe('Mongodb', () => {
  ...
  describe('upsertOne', () => {
    ...
    it('should return wrappewr data when upsertOne success', async () => {
      sinon.stub(Mongo.prototype, 'findOne').resolves({
        data: 'data'
      });
      stubDb.returns({
  ...
  describe('aggregateData', () => {
    it('should return wrapper error when get connection error', async () => {
      ...
      const res = await Mongo.prototype.aggregateData({});
      assert.equal(res.err.message, 'error');
    });
    it('should return wrapper error when db function is error', async () => {
      stubMongoConn.resolves({
    ...
    it('should return wrapper data when aggregateData not success', async () => {
      stubDb.returns({
        collection : () => {
          return {
    ...
    it('should return wrapper data when aggregateData success', async () => {
      stubDb.returns({
        collection : () => {
          return {
            aggregate: sinon.stub().callsFake(() => {
              return {
                toArray: () => {
                  return Promise.resolve([{ $group: { '_id': {'type': '$type'} } }]);
                }
              };
            })
          };
      ...
      const res = await Mongo.prototype.aggregateData({});
      assert.equal(res.data[0].$group._id.type, '$type');
      stubDb.restore();
    });
  });
  ...
  describe('findAggregateData', () => {
    it('should return wrapper error when get connection error', async () => {
      ...
      const res = await Mongo.prototype.findAggregateData({});
      assert.equal(res.err.message, 'error');
    });
    it('should return wrapper error when db function is error', async () => {
      stubMongoConn.resolves({
    ...
    it('should return wrapper data when findAggregateData not success', async () => {
      class MockCursor {
        skip() {
          return new MockCursor();
        }
      ...
      const res = await Mongo.prototype.findAggregateData({});
      assert.equal(res.err, 'Error Find Many Mongo db.aggregate is not a function');
      stubDb.restore();
    });
    it('should return wrapper data when findAggregateData success', async () => {
      class MockCursor {
        skip() {
          return new MockCursor();
        }
